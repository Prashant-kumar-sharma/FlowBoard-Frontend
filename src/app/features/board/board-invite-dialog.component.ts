...
export interface BoardInviteDialogData {
  currentUserId: number | null;
  existingMemberIds: number[];
}

export interface BoardInviteDialogResult {
  user: User;
  role: BoardMember['role'];
}

@Component({
  selector: 'app-board-invite-dialog',
  standalone: true,
  imports: [
    CommonModule,
    ReactiveFormsModule,
  ...
  template: `
    <h2 mat-dialog-title class="text-xl font-semibold">Add Board Member</h2>

    <form [formGroup]="form" (ngSubmit)="search()">
      <mat-dialog-content class="w-[min(92vw,36rem)] pt-2">
        <div class="grid gap-4">
          <div class="grid gap-3 sm:grid-cols-[minmax(0,1fr)_10rem_auto] sm:items-start">
...
export class BoardInviteDialogComponent {
  readonly form = this.fb.group({
    query: ['', [Validators.required, Validators.maxLength(80)]],
    role: ['MEMBER' as BoardMember['role'], Validators.required],
  });
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<BoardInviteDialogComponent>,
    @Inject(MAT_DIALOG_DATA) public data: BoardInviteDialogData
  ) {}
  ...
  selectUser(user: User): void {
    ...
    } satisfies BoardInviteDialogResult);
  }
}
